...
This data-set is open for everybody. Use it, share it, improve it, do what you want. And if you do, please cite this github repository as it's source.
...
```{r results_property2}
knitr::kable(Results %>% group_by(TournamentID) %>% summarise(n = n(), max = max(Rank), ranks = length(unique(Rank))) %>% ungroup() %>% filter(n != max))
```
